...
**Goal:** Incremental download с хешированием и прогрессом.

- [ ] `FileIndexService.cs` — SHA-256 хеши для incremental updates (вместо MD5)
- [ ] `ProfileManager.cs` — сохранение/загрузка профилей настроек (JSON)
- [ ] Content-Disposition validation для скачанных файлов
- [ ] `IProgress<T>` integration для отчёта о прогрессе загрузки
- [ ] End-to-end integration: GitHub API → download → verify → install
- [ ] Unit-тесты для всех edge cases

**Acceptance:** Полный цикл скачивания и установки локализации.
