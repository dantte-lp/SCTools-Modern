...
**Goal:** Финализация, CI/CD, документация, готовность к выпуску.

- [x] `Directory.Build.props` — shared version metadata (Version 2.0.0, Company, Copyright, RepositoryUrl)
- [x] `SCTools.App.csproj` — убран hardcoded RuntimeIdentifier (передаётся через `-r` в CI)
- [x] `SCTools.App.csproj` — добавлен Description, IncludeNativeLibrariesForSelfExtract, условный ApplicationIcon
- [x] `build.yml` — Linux Core job (fast) + Windows matrix (win-x64, win-arm64), coverage upload
- [x] `build.yml` — env vars (DOTNET_NOLOGO, DOTNET_CLI_TELEMETRY_OPTOUT), `needs: core` dependency
- [x] `release.yml` — Velopack CLI: `vpk download` (delta) → `vpk pack` → `vpk upload github`
- [x] `release.yml` — version extraction из git tag, `--channel` per-runtime, `--merge` for multi-arch
- [x] `release.yml` — `workflow_dispatch` trigger с ручным вводом тега
- [x] `README.md` — полный README (en): badges, features, installation, building, architecture, tech stack
- [x] `README.ru.md` — русская версия README
- [x] `SECURITY.md` — политика сообщения об уязвимостях, security practices
- [x] `LICENSE` — MIT License
- [x] Удалён стартовый debug-скрипт `Program.cs` из корня
- [x] Final analyzer: Core 0W/0E, 287 tests green, format clean

**Remaining for production release:**
- Code signing (SignPath Foundation / Azure Trusted Signing) — requires certificate
- Application icon (`Resources/app.ico`) — requires designer
- Screenshots in README — requires Windows build
- Performance benchmark — requires Windows + old version comparison

**Acceptance:** CI/CD полностью настроен, документация готова, Velopack packaging. **DONE.**
...
| 8 | 14 | 14 | Done. CI/CD (build+release), Velopack packaging, README (en/ru), SECURITY, LICENSE |
